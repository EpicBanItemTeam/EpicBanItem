...
public final class BlockUtils {
  ...
  private static final Map<Predicate<BlockSnapshot>, Function<BlockSnapshot, ItemStack>>
  ...
  public static Optional<ItemStack> fromBlock(BlockSnapshot snapshot) {
    try {
      return Optional.of(
          BLOCK_TO_ITEM.entrySet().stream()
              .filter(it -> it.getKey().test(snapshot))
              .map(it -> it.getValue().apply(snapshot))
              .findAny()
              .orElseGet(() -> ItemStack.builder().fromBlockSnapshot(snapshot).build()));
