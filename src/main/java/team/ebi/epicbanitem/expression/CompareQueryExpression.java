...
public abstract class CompareQueryExpression implements QueryExpression {
    ...
    protected CompareQueryExpression(DataView data, DataQuery query, BiPredicate<Double, Double> predicate) {
        this.value = data.get(query).flatMap(Coerce::asDouble).orElseThrow(InvalidDataException::new);
        this.predicate = predicate;
    }
